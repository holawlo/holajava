...
import collectionscomparator.CollectionComparisonComparator;
import collectionscomparator.CollectionComparisonResult;
import collectionscomparator.CollectionComparisonUtil;
...
public class CollectionComparisonUtilTest {
    ...
    void compareSimpleCollections() { //zakładamy, że wartości w stringu są tym samym co liczbowe -> "1" == 1
        ...
        Assertions.assertEquals(result.getCommon().get("2").intValue(), 2);
        Assertions.assertEquals(result.getCommon().get("3").intValue(), 3);
    }
