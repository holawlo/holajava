...
//        nameHolders.add(pensioner); - nie implementuje tego interfejsu
    ...
    private static void printDataProperWay(List<Person> peopleInfo) { //rzutowanie, polimorfizm, bo student jest personem
        for (Person person : peopleInfo) {
            System.out.println(person.showIncome());
            if (person instanceof Worker) {
                ((Worker) person).introductionInner();//bedzie exception jesli sie wpisze to bez instanceof
